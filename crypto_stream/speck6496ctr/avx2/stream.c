...
{
  int i;
  u32 nonce[2],K[3],key[26],C,B,A,x,y;
...
int ExpandKey(u32 K[],__m256i rk[], u32 key[])
{
  ...
  for(i=0;i<26;i+=2){
    rk[i]=SET(A,A,A,A,A,A,A,A); key[i]=A;
    B=RCS(B,8); B+=A; B^=i; A=LCS(A,3); A^=B;
    rk[i+1]=SET(A,A,A,A,A,A,A,A); key[i+1]=A;
    C=RCS(C,8); C+=A; C^=(i+1); A=LCS(A,3); A^=C;
  }
