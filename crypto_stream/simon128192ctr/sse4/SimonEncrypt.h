...
#define Ex6(X,Y,k) (Rx6(X,Y,k[0],k[1]),   Rx6(X,Y,k[2],k[3]),   Rx6(X,Y,k[4],k[5]),   Rx6(X,Y,k[6],k[7]),   Rx6(X,Y,k[8],k[9]), \
		    ...
		    Rx6(X,Y,k[60],k[61]), Rx6(X,Y,k[62],k[63]), Rx6(X,Y,k[64],k[65]), Rx6(X,Y,k[66],k[67]), Sx6(X,Y,k[68]))
...
#define RK(c0,c1,c2,A,B,C,rk,key,i) (A^=c0^(RCS(C,3)^RCS(C,4)), key[i]=A,   rk[i]=SET(A,A), \
				     B^=c1^(RCS(A,3)^RCS(A,4)), key[i+1]=B, rk[i+1]=SET(B,B), \
				     C^=c2^(RCS(B,3)^RCS(B,4)), key[i+2]=C, rk[i+2]=SET(C,C))
